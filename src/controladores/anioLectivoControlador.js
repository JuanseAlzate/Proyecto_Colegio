...
const validarFechas = (fecha_inicio, fecha_fin) => {
  return new Date(fecha_inicio) < new Date(fecha_fin);
};
...
const crearRegistro = async (req, res) => {
  try {
    ...
    if (!validarFechas(req.body.fecha_inicio, req.body.fecha_fin)) {
      return res.status(400).json({
...
const actualizarRegistro = async (req, res) => {
  try {
    ...
    const fechaInicioFinal = req.body.fecha_inicio || registro.fecha_inicio;
    const fechaFinFinal = req.body.fecha_fin || registro.fecha_fin;

    if (!validarFechas(fechaInicioFinal, fechaFinFinal)) {
      return res.status(400).json({
        mensaje: 'La fecha de inicio debe ser menor que la fecha de fin',
        resultado: null,
      });
